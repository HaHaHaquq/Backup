...
//                platform_printf("CMD_INPUT\r\n");
            ...
            case EVENT_ID_UART_BURN_START:
            {
		        ...
		        Buf = (uint32_t*)malloc(downdloader.mate.page_size);	
				if (NULL == Buf) {
...
//                        vTaskDelay(pdMS_TO_TICKS(10));
                    ...
                    {          
						...
						*p = ing_buner.block.loadaddr + ing_buner.sector_index*downdloader.mate.sector_size;
						...
						{
                            //开始读取数据
                            uint32_t res = flash_read(&self_addr, Buf, downdloader.mate.page_size);
							if (res) {
							    platform_printf("Flash read err\r\n");
...
//								UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_FILE_ERR);
							...
							{
								*(uint16_t*)(p+1) = downdloader.mate.page_size; //可能出错
                        ...
                        // platform_printf("si is %d\n",ing_buner.sector_index);
...
//								UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_FILE_ERR);
						...
						{
							ing_buner.total_burn_data_size += downdloader.mate.page_size;
							crc = check_crc16_918((uint8_t *)Buf,downdloader.mate.page_size);
							driver_append_tx_data((uint8_t *)Buf, downdloader.mate.page_size/2);
//							vTaskDelay(pdMS_TO_TICKS(20));
							driver_append_tx_data((uint8_t *)(Buf+downdloader.mate.page_size/2), downdloader.mate.page_size/2);
							driver_append_tx_data((uint8_t *)&crc, 2);
							ing_buner.uart_burn_step = 2;
							 ing_buner.pag_index += 1;  
							//printf("crc:%x\r\n",crc);
