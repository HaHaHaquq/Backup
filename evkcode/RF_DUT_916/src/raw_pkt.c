...
{
    raw_cnt_ctrl.run = 1;
    platform_printf("Raw Tx/Rx started\n");
...
{
    raw_cnt_ctrl.run = 0;
    platform_printf("Raw Tx/Rx stopped\n");
}
...
{
    if(ema_filter.corr_rssi < POWER_THRESHOLDS)return 0;
...
uint8_t set_rf_channel(uint8_t *ch)
{
    ...
    raw_cnt_ctrl.ch_id = (int8_t)(ch_id - default_ch)/2;
    DBG_PRINTF("CHID IS : %d\n", raw_cnt_ctrl.ch_id);
